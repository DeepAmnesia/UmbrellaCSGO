...
namespace Interfaces
{
    ...
    template<typename T>
    static T* get_interface2(const char* module, const char* name) {
        ...
        const unsigned int target = CreateInterfaceFn + *reinterpret_cast<unsigned*>(CreateInterfaceFn + 0x5) + 0x9;
    ...
    void Initialize()
    {
        ...
        auto matchmaking      = get_module_factory(GetModuleHandleW(L"matchmaking.dll"));
        auto f_system_factory = get_module_factory(GetModuleHandleW(L"filesystem_stdio.dll"));
      ...
      //g_GameTypes           = get_interface<IGameTypes>          (matchmaking     , "VENGINE_GAMETYPES_VERSION");
        g_FileSystem          = get_interface2<IFileSystem>("filesystem_stdio.dll", "VFileSystem");
        g_Server              = get_interface2<IServerGameDLL>("server.dll", "ServerGameDLL");
        g_StudioRender        = get_interface2<IStudioRender>("studiorender.dll", "VStudioRender");
