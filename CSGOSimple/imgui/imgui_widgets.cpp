...
bool ImGui::Combo(const char* label, int* current_item, std::vector<const char*> arr)
{
    int items_count = arr.size();
  
    bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)arr.data(), items_count, items_count);
    return value_changed;
}
//-------------------------------------------------------------------------
...
bool ImGui::Keybind(const char* str_id, int& key, int& mode)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 label_size = CalcTextSize(KeyNames[key]);
    ...
    else if (true)
    {
        ...
        if (popup_open) {
            SetNextWindowSize(ImVec2(100, ImGui::CalcMaxPopupHeightFromItemCount(6)));
            ...
            PopStyleVar();
            Spacing();
            if (Selectable("Toggle", mode == 0))
                mode = 0;
            if (Selectable("Hold", mode == 1))
                mode = 1;
            if (Selectable("Force Disable", mode == 2))
                mode = 2;
            if (Selectable("Always On", mode == 3))
                mode = 3;
            Spacing();
            EndPopup();
        }
    ...
    window->DrawList->AddText(frame_bb.Min, ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, g.Style.Alpha), buf_display);
    //PopFont();
