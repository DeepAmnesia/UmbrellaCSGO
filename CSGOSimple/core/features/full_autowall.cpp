...
void IEngineTrace::TraceLine(const Vector src, const Vector dst, int mask, IHandleEntity* entity, int collision_group, CGameTrace* trace) {
	std::uintptr_t filter[4] = {
		*reinterpret_cast<std::uintptr_t*>(g_cheat.m_TraceFilterSimple),
...
{
	m_PenetrationData.m_Weapon = g_LocalPlayer->m_hActiveWeapon();
...
bool C_AutoWall::HandleBulletPenetration()
{
	...
	int nEnterMaterial = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_EnterTrace.surface.surfaceProps)->game.material;
	...
	if (!TraceToExit(m_PenetrationData.m_EnterTrace.endpos, vecEnd, &m_PenetrationData.m_EnterTrace, &m_PenetrationData.m_ExitTrace))
		if (!(g_EngineTrace->GetPointContents(vecEnd, 0x600400B) & 0x600400B))
			return false;

	float flDamLostPercent = 0.16;
	float flDamageModifier = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_EnterTrace.surface.surfaceProps)->game.flDamageModifier;
	float flPenetrationModifier = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_EnterTrace.surface.surfaceProps)->game.flPenetrationModifier;

	int iEnterMaterial = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_EnterTrace.surface.surfaceProps)->game.material;
	...
	else if (iEnterMaterial == CHAR_TEX_FLESH && g_CVar->FindVar("ff_damage_reduction_bullets")->GetFloat() == 0
		&& m_PenetrationData.m_EnterTrace.hit_entity && ((C_BasePlayer*)(m_PenetrationData.m_EnterTrace.hit_entity))->IsPlayer() && ((C_BasePlayer*)(m_PenetrationData.m_EnterTrace.hit_entity))->m_iTeamNum() == g_LocalPlayer->m_iTeamNum())
	{
		if (g_CVar->FindVar("ff_damage_reduction_bullets")->GetFloat() == 0)
			return true;

		flPenetrationModifier = g_CVar->FindVar("ff_damage_reduction_bullets")->GetFloat();
		flDamageModifier = g_CVar->FindVar("ff_damage_reduction_bullets")->GetFloat();
	}
	else
	{
		float flExitPenetrationModifier = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_ExitTrace.surface.surfaceProps)->game.flPenetrationModifier;
		float flExitDamageModifier = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_ExitTrace.surface.surfaceProps)->game.flDamageModifier;
	...
	int iExitMaterial = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_ExitTrace.surface.surfaceProps)->game.material;
...
bool C_AutoWall::TraceToExit(Vector vecStart, Vector& vecEnd, CGameTrace* pEnterTrace, CGameTrace* pExitTrace)
{
	...
	std::array < std::uintptr_t, 4 > aFilter
		=
	{
		*(std::uintptr_t*)(g_cheat.m_TraceFilterSimple),
	...
	while (flDistance <= 90.0f)
	{
		...
		// cache contents
		int nContents = g_EngineTrace->GetPointContents(vecEnd, 0x4600400B, nullptr);
		if (!nFirstContents)
		...
		// trace line
		g_EngineTrace->TraceLine(vecEnd, vecTraceEnd, 0x4600400B, g_LocalPlayer, NULL, pExitTrace);
		...
		if (pExitTrace->startsolid && pExitTrace->surface.flags & SURF_HITBOX)
		{
			std::array < uintptr_t, 5 > aSkipTwoEntities
				=
			{
				*(std::uintptr_t*)(g_cheat.m_TraceFilterSkipTwoEntities),
				(uintptr_t)(g_LocalPlayer),
				NULL,
				NULL,
			...
			g_EngineTrace->TraceRay(Ray, 0x600400B, (CTraceFilter*)(aSkipTwoEntities.data()), pExitTrace);
		...
		if (!pExitTrace->DidHit() || pExitTrace->startsolid)
		{
			if (pEnterTrace->hit_entity != g_EntityList->GetClientEntity(NULL))
			{
...
bool C_AutoWall::SimulateFireBullet()
{
	...
	std::array < uintptr_t, 5 > aSkipTwoEntities
		=
	{
		*(uintptr_t*)(g_cheat.m_TraceFilterSkipTwoEntities),
		(uintptr_t)(g_LocalPlayer),
		NULL,
		NULL,
		NULL
	...
	while (m_PenetrationData.m_PenetrationCount > 0)
	{
		...
		g_EngineTrace->TraceRay(Ray, MASK_SHOT_HULL | CONTENTS_HITBOX, (CTraceFilter*)(aSkipTwoEntities.data()), &m_PenetrationData.m_EnterTrace);
		...
		surfacedata_t* pEnterSurfaceData = g_PhysSurface->GetSurfaceData(m_PenetrationData.m_EnterTrace.surface.surfaceProps);
		...
		if (m_PenetrationData.m_EnterTrace.hit_entity)
		{
			...
			{
				if (!pPlayer->IsAlive() || pPlayer->m_iTeamNum() == g_LocalPlayer->m_iTeamNum())
					return false;
...
void C_AutoWall::ScaleDamage(CGameTrace Trace, float_t& flDamage)
{
	...
	if (((flDamage - (flDamage * flArmorRatio)) * (fl47 * flArmorBonusRatio)) > ((C_BasePlayer*)(Trace.hit_entity))->m_ArmorValue())
		flNewDamage = flDamage - (((C_BasePlayer*)(Trace.hit_entity))->m_ArmorValue() / flArmorBonusRatio);
...
void C_AutoWall::ClipTraceToPlayers(C_BasePlayer* pPlayer, Vector vecStart, Vector vecEnd, CGameTrace* Trace, CTraceFilter* pTraceFilter, uint32_t nMask)
{
	...
	float flRange = Math::DistanceToRay(pPlayer->world_space_center(), vecStart, vecEnd);
	if (flRange < -60.0f || flRange > 60.0f)
		return;

	g_EngineTrace->ClipRayToEntity(Ray, nMask, pPlayer, &NewTrace);
...
bool C_AutoWall::IsPenetrablePoint(Vector vecShootPosition, Vector vecTargetPosition)
{
	...
	CGameTrace Trace;
	g_EngineTrace->TraceLine(vecShootPosition, vecTargetPosition, MASK_SHOT_HULL | CONTENTS_HITBOX, g_LocalPlayer, NULL, &Trace);
