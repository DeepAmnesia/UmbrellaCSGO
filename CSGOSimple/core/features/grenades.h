...
public:
	struct data_t {
		...
		void perform_fly_collision_resolution(trace_t& trace) {
			...
			velocity *= Math::clamp< float >(surface_elasticity * 0.45f, 0.f, 0.9f);
		...
		bool draw() const {
			if (m_path.size() <= 1u
				|| TICKS_TO_TIME(g_LocalPlayer->m_nTickBase()) >= m_ExpireTime)
				return false;
			...
			for (auto i = 1u; i < m_path.size(); ++i) {
				...
				{
					Render::Get().RenderLine(prev_screen.x, prev_screen.y, cur_screen.x, cur_screen.y, Color(g_Options.color_grenade_trajectory), 1.0f);

					if (i == m_path.size() - 1 && !g_Options.grenade_timers)
						Render::Get().render_circle_3d_filled_border(std::get< Vector >(m_path.at(i)), 130 * 3, 130, Color(g_Options.color_grenade_circle), 1.f);
				}
			...
			if (!g_DebugOverlay->ScreenPosition(Vector(vecTimerPosition.x, vecTimerPosition.y, vecTimerPosition.z), vecScreenPosition))
			{
				std::string szIcon = "";
				switch (m_index)
				{
				case WEAPON_HEGRENADE: szIcon = "j"; break;
				case WEAPON_SMOKEGRENADE: szIcon = "k"; break;
				case WEAPON_FLASHBANG: szIcon = "i"; break;
				case WEAPON_DECOY: szIcon = "m"; break;
				case WEAPON_INCGRENADE: szIcon = "n"; break;
				case WEAPON_MOLOTOV: szIcon = "l"; break;
				}
				static auto size = Vector2D(35.0f, 5.0f);
				ImVec2 vecTextSize = m_BigIcons->CalcTextSizeA(20.0f, FLT_MAX, NULL, szIcon.c_str());
				Render::Get().RenderText(szIcon, ImVec2(vecScreenPosition.x - vecTextSize.x / 2 + 1, vecScreenPosition.y - size.y * 0.5f - 18 - vecTextSize.y / 4), 20.f, Color::White, false, true, m_BigIcons);

				if (g_Options.grenade_timers)
				{
					float_t flPercentage = (m_ExpireTime - TICKS_TO_TIME(g_LocalPlayer->m_nTickBase())) / (m_ExpireTime - m_SourceTime);
					Render::Get().render_circle_3d_filled_border_progress(vecTimerPosition, 130 * 3, 130, Color(g_Options.color_grenade_circle), 2.f, flPercentage);
					Render::Get().RenderCircleFilled(vecScreenPosition.x, vecScreenPosition.y - size.y * 0.5f - 12, 21, 60, Color(0.1f, 0.1f, 0.1f, 0.5f));
					Render::Get().two_sided_arc(vecScreenPosition.x, vecScreenPosition.y - size.y * 0.5f - 12, 20, 1.f - flPercentage, Color(0.7f, 0.7f, 0.7f, 0.7f), 3);
				}
			}
...
public:
	...
	virtual void OnCreateMove(CUserCmd* cmd);
