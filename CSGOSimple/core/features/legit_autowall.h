...
namespace autowall 
{
	...
	void scale_damage(C_BasePlayer* local_player, CGameTrace& enter_trace, C_CSWeaponData* weapon_data, float& current_damage);
	...
	bool handle_bullet_penetration(C_BasePlayer* local_player, C_CSWeaponData* weapon_data, trace_t& enter_trace, Vector& eye_position, Vector direction, int& possible_hits_remaining, float& current_damage, float penetration_power, float ff_damage_bullet_penetration, bool pskip = false);
	...
	/*
	inline float GetPointDamage(Vector vecShootPosition, Vector vecTargetPosition)
	{
		const auto weapon = g_LocalPlayer->m_hActiveWeapon();

		if (!weapon) return false;

		const auto data = weapon->GetWeaponData();
		...
		QAngle angles;
		Vector direction;
		const Vector tmp = vecTargetPosition - vecShootPosition;

		Math::VectorAngles(tmp, angles);
		Math::AngleVectors(angles, direction);
		direction.Normalize();

		if (fire_bullet(g_LocalPlayer, vecShootPosition, weapon, direction, current_damage, entity))
