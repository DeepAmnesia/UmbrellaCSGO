...
void autowall::scale_damage(C_BasePlayer* local_player, CGameTrace& enter_trace, C_CSWeaponData* weapon_data, float& current_damage)
{

	if (!enter_trace.hit_entity || !enter_trace.hit_entity->GetClientClass() || enter_trace.hit_entity->GetClientClass()->m_ClassID != classids::get("CCSPlayer"))
		return;

	const auto target = reinterpret_cast<C_BasePlayer*>(enter_trace.hit_entity);
	const int hitgroup = enter_trace.hitgroup;
	const auto is_zeus = local_player->m_hActiveWeapon()->get_idx() == WEAPON_TASER;
	...
	if (is_armored()) 
	{
		...
		if ((current_damage - current_damage * armor_ratio) * (modifier * armor_bonus_ratio) > target->m_ArmorValue())
			new_damage = current_damage - target->m_ArmorValue() / armor_bonus_ratio;
...
{
	static const auto filter_simple = *reinterpret_cast<uint32_t*>(Utils::PatternScan(GetModuleHandleA("client.dll"), "55 8B EC 83 E4 F0 83 EC 7C 56 52") + 0x3d);
...
bool autowall::trace_to_exit(C_BasePlayer* local_player, trace_t& enter_trace, trace_t& exit_trace, const Vector start_position, const Vector direction, const bool is_local) 
{
	...
	while (current_distance <= max_distance) 
	{
		...
		if (!first_contents)
			first_contents = g_EngineTrace->GetPointContents(start, MASK_SHOT_HULL | CONTENTS_HITBOX);

		const auto point_contents = g_EngineTrace->GetPointContents(start, MASK_SHOT_HULL | CONTENTS_HITBOX);
		...
		if (!(point_contents & MASK_SHOT_HULL) || (point_contents & CONTENTS_HITBOX && point_contents != first_contents)) 
		{
			...
			g_EngineTrace->TraceRay(r, MASK_SHOT_HULL | CONTENTS_HITBOX, reinterpret_cast<CTraceFilter*>(filter), &exit_trace);
...
bool autowall::handle_bullet_penetration(C_BasePlayer* local_player, C_CSWeaponData* weapon_data, trace_t& enter_trace, Vector& eye_position, Vector direction, int& possible_hits_remaining, float& current_damage, float penetration_power, float ff_damage_bullet_penetration, bool pskip)
{
	...
	const bool is_solid_surf = (enter_trace.contents & CONTENTS_GRATE);
	const bool is_light_surf = (enter_trace.surface.flags & SURF_NODRAW);
	...
	{
		if (!(g_EngineTrace->GetPointContents(enter_trace.endpos, 0x600400B, nullptr) & 0x600400B))
			return false;
	}
	...
	const float exit_surf_penetration_modifier = exit_surface_data->game.flPenetrationModifier;
...
void autowall::fix_trace_ray(Vector end, Vector start, trace_t* oldtrace, C_BasePlayer* entity) 
{
	...
	const auto pos(center + entity->m_vecOrigin());
	...
	if (range <= 60.f) 
	{
		...
		trace_t trace;
		g_EngineTrace->ClipRayToEntity(ray, MASK_SHOT_HULL | CONTENTS_HITBOX, entity, &trace);
...
void autowall::clip_trace_to_players(const Vector& vec_abs_start, const Vector& vec_abs_end, uint32_t mask, ITraceFilter* filter, trace_t* tr) 
{
	...
	const Vector delta(vec_abs_end - vec_abs_start);
	...
	for (int i = 1; i <= 64; ++i) 
	{
		...
		Math::angle_matrix(entity->m_angAbsAngles(), entity->m_vecOrigin(), coordinate_frame);
		...
		const auto range_along = delta.Dot(extend);
		...
		if (range > 0.0f && range <= max_range) 
		{
			...
			g_EngineTrace->ClipRayToEntity(ray, MASK_SHOT_HULL | CONTENTS_HITBOX, entity, &player_trace);
...
{
	return hit_entity == g_EntityList->GetClientEntity(NULL);
}
...
bool autowall::fire_bullet(C_BasePlayer* local_player, Vector eyepos, C_BaseCombatWeapon* weapon, Vector& direction, float& current_damage, C_BasePlayer* entity) 
{
	...
	while (last_penetrated_count > 0 && current_damage >= 1.0f) 
	{
		...
		const auto enter_surface_data = g_PhysSurface->GetSurfaceData(enter_trace.surface.surfaceProps);
		const float enter_surf_penetration_modifier = enter_surface_data->game.flPenetrationModifier;
		...
		const bool can_do_damage = (enter_trace.hitgroup > 0 && enter_trace.hitgroup <= 8);
		const bool is_player = (enter_trace.hit_entity && enter_trace.hit_entity->GetClientClass() && enter_trace.hit_entity->GetClientClass()->m_ClassID == classids::get("CCSPlayer"));
		const bool is_enemy = !reinterpret_cast<C_BasePlayer*>(enter_trace.hit_entity)->m_iTeamNum() == g_LocalPlayer->m_iTeamNum();// is_teammate(local_player);
		const bool on_team = (reinterpret_cast<C_BasePlayer*>(enter_trace.hit_entity)->m_iTeamNum() == 2 || reinterpret_cast<C_BasePlayer*>(enter_trace.hit_entity)->m_iTeamNum() == 3);
...
float autowall::can_hit(C_BasePlayer* local_player, Vector& point, C_BasePlayer* entity) 
{
	...
	QAngle angles;
	Vector direction;
	const Vector tmp = point - local_player->get_eye_position();
	...
	const auto local_weapon = reinterpret_cast<C_BaseCombatWeapon*>(g_EntityList->GetClientEntityFromHandle(local_player->m_hActiveWeapon()));
